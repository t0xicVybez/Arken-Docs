...
Manage game servers running under Ronin Server Manager (RSM) directly from Discord. Supports starting, stopping, sending console commands, checking player lists, and a live auto-updating status embed.
...
Uninstall the addon from the Addon Manager to remove all `/rsm` commands from your server.
